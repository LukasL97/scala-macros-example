...
During the compilation, a macro annotation will expand the *abstract syntax tree (AST)* of the annotated definition.
...
Compared to Scala 2, their usage is still clumsy.
Hence, we will first have a look at macro annotations in Scala 2 and afterwards compare the example
to a (mostly) equivalent implementation in Scala 3.

The complete code can be found [here](https://github.com/LukasL97/scala-3-macros-example).
...
The cache will store key-value pairs of the input parameters of the method as well as its return value and directly return them for cached inputs.
...
Then, we define some variable names in (3), which we will also use on the right-hand side.
...
With this in place, we can annotate arbitrary methods via `@cached`.
...
================================================================================
TODO: ich würde kein implicit object nehmen sondern lieber ein `implicit val`.  Das `val` kann man annotaten mit dem Typ und das verhindert ein paar edgecases im compiler mit implicit.  Daher ist das der bessere stil (mMn)
================================================================================
